...
/**
 * @brief During training only, sets a random nested portion of @f$x@f$ to 0, adjusting
 *        the rest of the vector magnitude accordingly.
 *
 * @param bottom input Blob vector (length 1)
 *   -# @f$ (N \times C \times H \times W) @f$
...
class NestedDropoutLayer : public NeuronLayer<Dtype> {
 public:
   ...
   *     Sets the probability @f$ p @f$ that any given unit is dropped.
   */
  explicit NestedDropoutLayer(const LayerParameter& param)
      : NeuronLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_NESTED_DROPOUT;
  }
